...
def get_supplier_categories():
    ...
    settings = _get_settings()
    return settings.get("supplierCategories", [])


@router.post("/supplier-categories", status_code=201)
def create_supplier_category(payload: ConfigItem):
    """Yeni tedarikçi kategorisi ekle"""
    settings = _get_settings()
    items = settings.get("supplierCategories", [])
    
    # Aynı isimde varsa hata ver
    if any(i.get("name", "").lower() == payload.name.lower() for i in items):
    ...
    items.append(new_item)
    settings["supplierCategories"] = items
    save_json("settings.json", settings)
    return new_item


@router.put("/supplier-categories/{item_id}")
def update_supplier_category(item_id: str, payload: ConfigItem):
    """Tedarikçi kategorisini güncelle"""
    settings = _get_settings()
    items = settings.get("supplierCategories", [])
    ...
    raise HTTPException(status_code=404, detail="Tedarikçi kategorisi bulunamadı")


@router.delete("/supplier-categories/{item_id}")
def delete_supplier_category(item_id: str):
    """Tedarikçi kategorisini sil"""
    settings = _get_settings()
    items = settings.get("supplierCategories", [])
    items = [i for i in items if i.get("id") != item_id]
    settings["supplierCategories"] = items
