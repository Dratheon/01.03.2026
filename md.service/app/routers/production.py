...
def reschedule_order(order_id: str, payload: RescheduleOrder):
    ...
    # Bildirim oluştur - Üretim siparişi ertelendi
    if is_postponed:
        supplier_name = order.get("supplierName", "")
        job_id = order.get("jobId", "")
        create_system_notification(
            notification_type="order_rescheduled",
            title="Üretim Siparişi Ertelendi",
            message=f"{supplier_name} - {delay_days} gün ertelendi ({new_date})",
            link=f"/uretim/siparisler?id={order_id}",
            related_id=order_id,
        )
