...
def get_user_info(authorization: Optional[str] = None) -> tuple:
    """
    Token'dan kullanıcı bilgisi al - helper fonksiyon.
    Diğer router'lar için merkezi fonksiyon.
...
async def login(request: LoginRequest):
    ...
    # === ROL BAZLI İZİNLER ===
    # İzinler tamamen rol bazlı - kullanıcı özel izinleri kullanılmıyor
    # Roller sayfasından yönetilir (roles.json)
    role_permissions = []
    
    # Kullanıcının rolünü bul (role adıyla eşleştir)
    user_role = user.get("role")
    if user_role:
        roles = load_json("roles.json")
        for r in roles:
            if r.get("ad") == user_role and not r.get("deleted"):
                role_permissions = r.get("permissions", [])
                break
    # ==========================================
    ...
    # Session kaydet (birleştirilmiş izinlerle)
    active_sessions[token] = {
        "user": {
            ...
            "role": user.get("role"),
            "permissions": role_permissions,  # Rol bazlı izinler (roles.json'dan)
