...
def _update_service_task(task_id: str, payload) -> dict:
    """Servis görevini güncelle (jobs.json'a yaz)"""
    job_id = task_id.replace("SRV-", "")
    jobs = load_json("jobs.json")
    
    job = next((j for j in jobs if j.get("id") == job_id), None)
    if not job:
        raise HTTPException(status_code=404, detail="Servis işi bulunamadı")
    
    # Ekip ve personel ataması
    if hasattr(payload, 'teamId') and payload.teamId is not None:
        job["assignedTeamId"] = payload.teamId
    if hasattr(payload, 'teamName') and payload.teamName is not None:
        job["assignedTeamName"] = payload.teamName
    if hasattr(payload, 'assignedPersonnel') and payload.assignedPersonnel is not None:
        job["assignedPersonnel"] = payload.assignedPersonnel
    
    # Planlanan tarih (servis randevusu)
    if hasattr(payload, 'plannedDate') and payload.plannedDate is not None:
        if "service" not in job:
            job["service"] = {}
        job["service"]["appointmentDate"] = payload.plannedDate
        
        # Durum güncelle
        if job.get("status") == "SERVIS_RANDEVU_BEKLIYOR":
            job["status"] = "SERVIS_RANDEVULU"
    
    job["updatedAt"] = _now()
    
    # Kaydet
    job_idx = next(i for i, j in enumerate(jobs) if j.get("id") == job_id)
    jobs[job_idx] = job
    save_json("jobs.json", jobs)
    
    # Güncellenmiş task formatında döndür
    service_tasks = _get_service_tasks()
    updated_task = next((t for t in service_tasks if t.get("id") == task_id), None)
    return updated_task or {"success": True}


def _report_service_issue(task_id: str, payload) -> dict:
    """Servis görevi için sorun bildir"""
    job_id = task_id.replace("SRV-", "")
    jobs = load_json("jobs.json")
    ...
    # issues array yoksa oluştur
    if "issues" not in job:
        job["issues"] = []
    ...
    # Yedek sipariş oluştur (isteğe bağlı)
    if payload.createReplacement:
        ...
        replacement_order = {
            "id": _gen_id("PROD"),
            "jobId": job.get("id"),
            "jobTitle": f"{job.get('customerName')} - Servis Yedek",
            ...
            "items": [{
                ...
                "unit": "adet",
                "combination": None,
                "notes": f"Servis Yedek - {payload.note or 'Servis sorunu'}",
                "receivedQty": 0,
                "problemQty": 0,
                "isReplacement": True,
            ...
            "notes": f"Servis sorunu için yedek sipariş. Görev: {task_id}",
            "status": "pending",
            "issues": [],
            "deliveryHistory": [],
            "createdAt": _now(),
        ...
        production_orders.append(replacement_order)
...
def _resolve_service_issue(task_id: str, issue_id: str) -> dict:
    """Servis görevi için sorunu çöz"""
    job_id = task_id.replace("SRV-", "")
    jobs = load_json("jobs.json")
    ...
    # Sorunu bul
    issue = next((i for i in job.get("issues", []) if i.get("id") == issue_id), None)
    if not issue:
    ...
    # Güncellenmiş task formatında döndür
    service_tasks = _get_service_tasks()
    return next((t for t in service_tasks if t.get("id") == task_id), {"success": True})


def _get_service_tasks() -> list:
    """Servis işlerini assembly task formatına çevir"""
    jobs = load_json("jobs.json")
    
    # Aktif servis işlerini filtrele (tamamlanmamış olanlar)
    service_jobs = [j for j in jobs if 
        j.get("startType") == "SERVIS" and 
        j.get("status") not in ["SERVIS_KAPALI", "ANLASILAMADI"]
    ]
    
    service_tasks = []
    for job in service_jobs:
        # Servis randevu tarihini al
        service_data = job.get("service", {})
        current_visit = None
        
        # En son planlanan ziyareti bul
        visits = service_data.get("visits", [])
        for visit in visits:
            if visit.get("status") in ["scheduled", "in_progress"]:
                current_visit = visit
                break
        
        # Planlanan tarih: randevu tarihi veya ziyaret tarihi
        planned_date = None
        if current_visit:
            planned_date = current_visit.get("appointmentDate")
        elif service_data.get("appointmentDate"):
            planned_date = service_data.get("appointmentDate")
        
        # Durum belirleme
        status_map = {
            "SERVIS_RANDEVU_BEKLIYOR": "pending",
            "SERVIS_RANDEVULU": "planned",
            "SERVIS_YAPILIYOR": "in_progress",
            "SERVIS_DEVAM_EDIYOR": "in_progress",
            "SERVIS_ODEME_BEKLIYOR": "completed",
        }
        task_status = status_map.get(job.get("status"), "pending")
        
        # Assembly task formatında oluştur
        service_task = {
            "id": f"SRV-{job.get('id')}",  # Servis task ID'si
            "jobId": job.get("id"),
            "roleId": "SERVICE",
            "roleName": "Servis",
            "stageId": "SERVICE",
            "stageName": "Servis Görevi",
            "stageOrder": 1,
            ...
            "estimatedDate": planned_date,
            "plannedDate": planned_date,
            "startedAt": None,
            "completedAt": None,
            
            "teamId": job.get("assignedTeamId"),
            "teamName": job.get("assignedTeamName"),
            "assignedPersonnel": job.get("assignedPersonnel", []),
            ...
            "issues": job.get("issues", []),  # Job'daki sorunları dahil et
            "delays": [],
            "totalDelayDays": 0,
            "isDelayed": False,
            
            "isServiceTask": True,  # Servis görevi olduğunu işaretle
            "serviceFixedFee": service_data.get("fixedFee"),
            
            "createdAt": job.get("createdAt"),
            "updatedAt": job.get("updatedAt") or job.get("createdAt")
        }
        
        service_tasks.append(service_task)
...
def list_tasks(
    ...
    includeService: Optional[bool] = True  # Servis işlerini dahil et
):
    """Tüm montaj görevlerini listele (servis işleri dahil)"""
    tasks = load_json("assemblyTasks.json")
    
    # Servis işlerini dahil et
    if includeService:
        service_tasks = _get_service_tasks()
        tasks = tasks + service_tasks
...
def get_today_tasks(teamId: Optional[str] = None):
    """Bugünkü montaj görevleri (ekip görünümü için) - servis işleri dahil"""
    tasks = load_json("assemblyTasks.json")
...
def get_tasks_by_job(job_id: str):
    """Bir iş için tüm montaj görevleri - servis işleri dahil"""
    tasks = load_json("assemblyTasks.json")
...
def update_task(task_id: str, payload: UpdateAssemblyTask):
    """Görevi güncelle"""
    # Servis görevi mi kontrol et
    if task_id.startswith("SRV-"):
        return _update_service_task(task_id, payload)
...
def report_issue(task_id: str, payload: ReportIssue):
    """Montaj sorunu bildir (servis görevleri dahil)"""
    # Servis görevi kontrolü
    if task_id.startswith("SRV-"):
        return _report_service_issue(task_id, payload)
    
    # Normal montaj görevi
    tasks, idx, task = _find_task(task_id)
...
def resolve_issue(task_id: str, issue_id: str):
    """Sorunu çözüldü olarak işaretle (servis görevleri dahil)"""
    # Servis görevi kontrolü
    if task_id.startswith("SRV-"):
        return _resolve_service_issue(task_id, issue_id)
...
@router.get("/summary")
def get_summary():
    """Montaj özet istatistikleri - servis işleri dahil"""
    tasks = load_json("assemblyTasks.json")
    ...
    # Servis işleri ayrı istatistik
    service_count = len([t for t in tasks if t.get("isServiceTask")])
...
def check_team_availability(teamId: str, date: str):
    ...
    return {
        ...
        "warning": f"Bu ekip aynı gün {len(team_tasks)} başka görevde atanmış" if team_tasks else None
    }
...
def reschedule_task(task_id: str, payload: UpdateTaskWithDelay):
    ...
    # Bildirim oluştur - Montaj ertelendi
    if is_postponed:
        customer_name = task.get("customerName", "")
        role_name = task.get("roleName", "")
        job_id = task.get("jobId", "")
        create_system_notification(
            notification_type="assembly_rescheduled",
            title="Montaj Ertelendi",
            message=f"{role_name} - {customer_name} | {delay_days} gün ertelendi ({new_date})",
            link=f"/montaj/takvim?taskId={task_id}",
            related_id=task_id,
        )
