...
const SatinalmaTedarikciler = () => {
  ...
  // URL parametreleriyle modal açma desteği
  useEffect(() => {
    const openNew = searchParams.get('openNew');
    const nameParam = searchParams.get('name');
    
    if (openNew === 'true') {
      // Modal'ı aç ve varsa ismi doldur
      setEditing(null);
      setSupplierForm({
        ...defaultSupplierForm,
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError('');
      const [suppliersPayload, categoriesPayload] = await Promise.all([
        getSuppliersFromAPI(),
        getSupplierCategories(),
      ]);
      setSuppliers(suppliersPayload || []);
      setSupplierCategories(categoriesPayload || []);
    } catch (err) {
  ...
  const filteredSuppliers = useMemo(() => {
    ...
    if (search) {
      ...
      data = data.filter((s) => {
        // Kategori adını bul (id veya name ile eşleştir)
        const categoryObj = supplierCategories.find(c => c.id === s.category || c.name === s.category);
        const categoryLabel = categoryObj?.name || s.category || '';
        return (
    ...
    return data;
  }, [suppliers, supplierCategories, search, typeFilter]);
          ...
          columns={[
              {
                ...
                accessor: 'name',
                render: (_, row) => {
                  const categoryObj = supplierCategories.find(c => c.id === row.category || c.name === row.category);
                  const categoryLabel = categoryObj?.name || row.category || 'Genel';
                  return (
                    <div>
                      <div style={{ fontWeight: 600 }}>{row.name}</div>
                      <div className="text-muted" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        {categoryObj?.icon && <StatusIcon icon={categoryObj.icon} style={{ fontSize: 14 }} />}
                        {categoryLabel}
                      </div>
                    </div>
                  );
        ...
        <form id="supplier-form" onSubmit={saveSupplier} className="grid grid-2">
          ...
          <div className="form-group">
            ...
            <select
              ...
              onChange={(e) => setSupplierForm((p) => ({ ...p, supplyType: e.target.value }))}
            >
            ...
            <small className="text-muted" style={{ fontSize: '0.75rem', marginTop: 4 }}>
              Cam ve Dış Üretim tedarikçileri, iş kolları ayarlarından varsayılan olarak atanabilir.
            </small>
          </div>
          <div className="form-group">
            <label className="form-label">
              ...
              <div className="grid grid-4" style={{ gap: 16 }}>
                ...
                <div>
                  ...
                  <div style={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px' }}>
                    {(() => {
                      const cat = supplierCategories.find(c => c.id === selectedSupplier?.category);
                      return cat ? <><StatusIcon icon={cat.icon} style={{ fontSize: 16 }} /> {cat.name}</> : (selectedSupplier?.category || '-');
                    })()}
                  </div>
                </div>
                <div>
