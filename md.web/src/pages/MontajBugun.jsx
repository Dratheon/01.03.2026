...
const MontajBugun = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [jobsData, teamsData, settingsData, personnelData] = await Promise.all([
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                          {/* Servis görevleri için sorun bildirme */}
                          {task.isServiceTask ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                              <div style={{ 
                                padding: '4px 8px', 
                                ...
                                <StatusIcon icon="build" style={{ fontSize: 12, verticalAlign: 'middle', marginRight: 2 }} />
                                Servis
                              </div>
                              <button
      ...
      >
        {selectedTask && (
          <>
            ...
            <div className="form-group">
              <label className="form-label"><StatusIcon icon="person" style={{ marginRight: 4, verticalAlign: 'middle' }} /> Sorumlu Kişi *</label>
              <select
                className="form-control"
            ...
            <div className="form-group">
              ...
              {issueForm.photoUrl && (
                <img 
                  ...
                  alt="Sorun fotoğrafı" 
                  style={{ maxWidth: 200, marginTop: '0.5rem', borderRadius: 4, cursor: 'zoom-in' }}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '1.5rem' }}>
              ...
              <button 
                ...
                disabled={actionLoading || !issueForm.item || !issueForm.photoUrl || !issueForm.responsiblePersonId}
              >
      ...
      >
        <div className="modal-complete-content">
          ...
          <div className="modal-info-card">
            {selectedTask && (
              <>
                <div className="modal-info-title">{selectedTask.roleName} - {selectedTask.stageName}</div>
                <div className="modal-info-subtitle">{selectedTask.customerName}</div>
              </>
            )}
            {selectedJob && (
              <>
                <div className="modal-info-title">{selectedJob.customerName}</div>
                <div className="modal-info-subtitle">{selectedJob.tasks.length} görev tamamlanacak</div>
              </>
            )}
            {(selectedJob || (selectedTask && isLastStageInRole(selectedTask, jobs.find(j => j.tasks.some(t => t.id === selectedTask?.id))))) && (
              <div className="modal-info-warning">
                <StatusIcon icon="warning" style={{ fontSize: 16 }} /> Son aşama - Müşteri imzası zorunlu
              </div>
            )}
          </div>
          ...
          <div className="modal-form-grid">
            <div className="form-group">
              <label className="form-label">
                <StatusIcon icon="photo_camera" style={{ marginRight: 4, verticalAlign: 'middle' }} /> 
          ...
          <div className="form-group">
            <label className="form-label">
              <StatusIcon icon="draw" style={{ marginRight: 4, verticalAlign: 'middle' }} /> 
              Müşteri İmzası 
              {(selectedJob || (selectedTask && isLastStageInRole(selectedTask, jobs.find(j => j.tasks.some(t => t.id === selectedTask?.id))))) 
