...
const STATUS_LABELS = {
  draft: { label: 'Taslak', tone: 'secondary', icon: 'edit_note' },
  sent: { label: 'Gönderildi', tone: 'primary', icon: 'send' },
  partial: { label: 'Kısmi Teslim', tone: 'warning', icon: 'inventory' },
  delivered: { label: 'Tamamlandı', tone: 'success', icon: 'check_circle' },
};
...
const SatinalmaSiparisler = () => {
  ...
  // Product search - Seçilen tedarikçinin ürünlerinde ara
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        // Tedarikçi seçiliyse sadece o tedarikçinin ürünlerinde ara
        const results = await searchStockItems(productSearch, orderForm.supplierId || '');
        setProductResults(results.slice(0, 15));
      } catch (err) {
  ...
  }, [productSearch, orderForm.supplierId]);
  ...
  const summary = useMemo(() => {
    ...
    return { total, draft, pending, completed, totalAmount };
  }, [orders]);
  ...
  const saveDelivery = async (e) => {
    ...
    try {
      ...
      // İrsaliye dosyası varsa yükle (tedarikçi bilgisiyle)
      if (deliveryForm.waybillFile) {
        try {
          await uploadDocument(
            ...
            'irsaliye', 
            `Teslimat İrsaliyesi - ${selectedOrder.supplierName || ''} - ${selectedOrder.id} - ${new Date().toLocaleDateString('tr-TR')}`,
            null,
  ...
  return (
    <div>
        ...
        actions={
          <>
            {missingItems.length > 0 && (
              <button className="btn btn-warning" type="button" onClick={openFromMissing} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <StatusIcon icon="warning" style={{ fontSize: 16 }} />
                Eksik Ürünler ({missingItems.length})
              </button>
            )}
            <button className="btn btn-secondary" type="button" onClick={exportToCSV} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div className="kpi-cards-container">
        ...
        <div 
          className={`kpi-card ${statusFilter === 'draft' ? 'active' : ''}`}
        ...
        >
          ...
          <div className="kpi-info">
            <div className="kpi-label">Taslak</div>
            <div className="kpi-value">{formatNumber(summary.draft)}</div>
          </div>
        </div>

        <div 
          className={`kpi-card ${statusFilter === 'sent' || statusFilter === 'partial' ? 'active' : ''}`}
        ...
        <div 
          className={`kpi-card ${statusFilter === 'delivered' ? 'active' : ''}`}
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-info">
            <div className="kpi-label">Toplam Tutar</div>
            <div className="kpi-value" style={{ fontSize: 18 }}>{formatCurrency(summary.totalAmount)}</div>
          </div>
        </div>
      </div>
        ...
        <DataTable
          columns={[
              ...
              },
              {
                ...
                accessor: 'status',
                render: (val) => {
                  const status = STATUS_LABELS[val] || { label: val, tone: 'secondary', icon: 'help' };
                  return (
              ...
              },
              {
                ...
                render: (_, row) => (
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button className="btn btn-primary btn-small btn-icon" type="button" onClick={() => openDetail(row)} title="Detayları Görüntüle">
                      <StatusIcon icon="visibility" style={{ fontSize: 16 }} />
                    </button>
                    {row.status === 'draft' && (
                      <>
                        <button className="btn btn-success btn-small btn-icon" type="button" onClick={() => sendOrder(row.id)} title="Siparişi Gönder">
                          <StatusIcon icon="send" style={{ fontSize: 16 }} />
                        </button>
                        <button className="btn btn-danger btn-small btn-icon" type="button" onClick={() => setDeleteTarget(row)} title="Siparişi Sil">
      ...
      <Modal
        ...
        title={
          <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <StatusIcon icon="add_shopping_cart" />
        ...
        <form id="order-form" onSubmit={saveOrder}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setOrderForm((p) => ({
                    ...
                    items: [], // Tedarikçi değişince kalemleri sıfırla
                  }));
                  setItemForm(defaultItemForm); // Ürün formunu sıfırla
                  setProductSearch('');
                  setProductResults([]);
                }}
                required
          ...
          <div className="card subtle-card" style={{ padding: 16, marginBottom: 16 }}>
            <h4 style={{ margin: '0 0 16px 0', fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
              <StatusIcon icon="add_circle" style={{ fontSize: 16 }} />
              Ürün Ekle
            </h4>
            ...
            {/* Ürün Arama - Sadece ürün seçili değilse göster */}
            {!itemForm.productCode && (
              <div style={{ marginBottom: 16 }}>
                {/* Tedarikçi seçilmemişse uyarı göster */}
                {!orderForm.supplierId ? (
                  <div style={{
                    ...
                    <StatusIcon icon="warning" style={{ fontSize: 24, color: 'var(--color-warning)' }} />
                    <div>
                      <div style={{ fontWeight: 600, color: 'var(--color-warning)' }}>Önce tedarikçi seçin</div>
                      <div className="text-muted" style={{ fontSize: 12 }}>
                        Ürün aramak için yukarıdan bir tedarikçi seçmelisiniz
                      </div>
                    </div>
                  </div>
                ) : (
                  <>
                    <AutocompleteInput
                      label={`Ürün Ara (${orderForm.supplierName})`}
                      value={productSearch}
                      ...
                      displayKey="name"
                      valueKey="id"
                      placeholder={`${orderForm.supplierName} ürünlerinde ara...`}
                      minChars={2}
                      renderOption={(opt) => (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ fontWeight: 600, fontSize: 14 }}>
                              ...
                              <span className="text-muted">-{opt.colorCode}</span>
                            </div>
                            <div className="text-muted" style={{ fontSize: 12 }}>{opt.name} {opt.colorName || ''}</div>
                          </div>
                          <div style={{ textAlign: 'right', fontSize: 11 }} className="text-muted">
                            Stok: {opt.onHand || 0}
                          </div>
                        </div>
                      )}
                    />
                    <div className="text-muted" style={{ fontSize: 11, marginTop: 6, display: 'flex', alignItems: 'center', gap: 4 }}>
                      <StatusIcon icon="info" style={{ fontSize: 12 }} />
                      <strong>{orderForm.supplierName}</strong> tedarikçisinin ürünlerinde arama yapılıyor
                    </div>
                  </>
                )}
              </div>
            ...
            {/* Miktar ve Ekle Butonu - Sadece ürün seçiliyse göster */}
            {itemForm.productCode && (
              <div className="grid grid-3" style={{ gap: 12 }}>
                <div className="form-group" style={{ margin: 0 }}>
                  <label className="form-label" style={{ fontSize: 12 }}>Miktar</label>
                  <NumberInput
                    min={1}
                ...
                <div className="form-group" style={{ margin: 0 }}>
                  <label className="form-label" style={{ fontSize: 12 }}>Birim</label>
                  <select
                    className="form-select"
                    value={itemForm.unit}
                ...
                <div className="form-group" style={{ margin: 0, display: 'flex', alignItems: 'flex-end' }}>
                  <button
                    ...
                    disabled={!itemForm.productCode || itemForm.quantity <= 0}
                  >
                    <StatusIcon icon="add_circle" style={{ fontSize: 18 }} />
                    Listeye Ekle
                  </button>
                </div>
              </div>
          ...
          <div className="card subtle-card" style={{ padding: 16 }}>
            <h4 style={{ margin: '0 0 12px 0', fontSize: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
              <StatusIcon icon="inventory_2" style={{ fontSize: 16 }} />
              Sipariş Kalemleri ({orderForm.items.length})
            </h4>
      ...
      <Modal
        ...
        actions={
          <>
            <button className="btn btn-secondary" type="button" onClick={() => exportOrderToPDF(selectedOrder)} title="Siparişi PDF olarak yazdır" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <StatusIcon icon="print" style={{ fontSize: 16 }} />
              PDF Yazdır
            </button>
            {['sent', 'partial'].includes(selectedOrder?.status) && (
              <button className="btn btn-success" type="button" onClick={openDelivery} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <StatusIcon icon="inventory" style={{ fontSize: 16 }} />
                Teslimat Kaydet
              </button>
            )}
            {selectedOrder?.status === 'draft' && (
              <button className="btn btn-primary" type="button" onClick={() => sendOrder(selectedOrder.id)} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <StatusIcon icon="send" style={{ fontSize: 16 }} />
                Siparişi Gönder
              </button>
            )}
            ...
            <div className="card subtle-card" style={{ marginBottom: 16 }}>
              <div className="grid grid-4" style={{ gap: 16 }}>
                ...
                <div>
                  ...
                  <span className={`badge badge-${STATUS_LABELS[selectedOrder.status]?.tone || 'secondary'}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                    <StatusIcon icon={STATUS_LABELS[selectedOrder.status]?.icon || 'help'} style={{ fontSize: 14 }} />
                  ...
                  <tbody>
                    {selectedOrder.items?.map((item, idx) => {
                      ...
                      const statusBadge = remaining === 0 
                        ? { label: 'Tamam', tone: 'success', icon: 'check_circle' }
                        : remaining === item.quantity
                        ? { label: 'Bekliyor', tone: 'secondary', icon: 'hourglass_empty' }
                        : { label: 'Eksik', tone: 'warning', icon: 'warning' };
                      return (
                        <tr key={`${item.productCode}-${idx}`} style={{ background: remaining > 0 && remaining < item.quantity ? 'rgba(245, 158, 11, 0.05)' : undefined }}>
                          ...
                          <td style={{ 
                            ...
                            {remaining > 0 ? `-${remaining}` : (
                              <StatusIcon icon="check" style={{ fontSize: 16 }} />
                            )}
                          </td>
                          <td>
                            <span className={`badge badge-${statusBadge.tone}`} style={{ fontSize: 10, display: 'inline-flex', alignItems: 'center', gap: 2 }}>
                              <StatusIcon icon={statusBadge.icon} style={{ fontSize: 12 }} />
                              {statusBadge.label}
                            </span>
                          </td>
      ...
      <Modal
        ...
        actions={
          <>
            <button className="btn btn-secondary" type="button" onClick={() => setDeliveryOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <StatusIcon icon="close" style={{ fontSize: 16 }} />
              İptal
            </button>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 4 }}
            >
              <StatusIcon icon="done_all" style={{ fontSize: 16 }} />
              Tümünü Doldur
            </button>
            <button
              className="btn btn-success"
      ...
      >
        <form id="delivery-form" onSubmit={saveDelivery}>
          ...
          <div className="text-muted" style={{ marginBottom: 12, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <td>
                      {item.remaining > 0 ? (
                        ...
                        <span className="badge badge-success" style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>
                          <StatusIcon icon="check" style={{ fontSize: 12 }} />
                          Tamam
                        </span>
                      )}
                    </td>
          ...
          <div className="form-group" style={{ marginTop: 16 }}>
            <label className="form-label">
              <StatusIcon icon="attach_file" style={{ fontSize: 14, marginRight: 4 }} />
              İrsaliye / Belge Yükle (opsiyonel)
            </label>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <label 
                style={{
              ...
              >
                <StatusIcon icon="folder_open" style={{ fontSize: 16 }} />
                <span>Dosya Seç</span>
                <input
                  type="file"
              ...
              {deliveryForm.waybillFile && (
                <div style={{ 
                  ...
                  <StatusIcon icon="description" style={{ fontSize: 18, color: 'var(--color-primary)' }} />
                  <span style={{ fontSize: 13, fontWeight: 500, flex: 1 }}>{deliveryForm.waybillFile.name}</span>
                  <button
                    type="button"
                    className="btn btn-secondary btn-small"
                    style={{ padding: '4px 10px', fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}
                    onClick={() => previewLocalFile(deliveryForm.waybillFile)}
                    title="Önizle"
                  >
      ...
      <Modal
        ...
        title={
          <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <StatusIcon icon="warning" style={{ color: 'var(--color-warning)' }} />
            Eksik Ürünler - Toplu Sipariş
          </span>
        }
        size="xlarge"
        ...
        actions={
          <>
            <button className="btn btn-secondary" type="button" onClick={() => setMissingOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <StatusIcon icon="close" style={{ fontSize: 16 }} />
              Kapat
            </button>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 4 }}
            >
              <StatusIcon icon={submitting ? 'hourglass_empty' : 'shopping_cart'} style={{ fontSize: 16 }} />
