...
const defaultForm = {
  ...
  username: '',
  password: '',
  systemRole: 'user',  // Sistem rolü (admin/manager/user) - izinler rolden gelir
};
...
const Personnel = () => {
  ...
  const [roles, setRoles] = useState([]); // Dinamik roller (roles.json'dan)
  ...
  // Şifre değiştirme (düzenleme modalı içinde)
    ...
    const load = async () => {
      try {
        ...
        const [personnelData, rolesData, usersData] = await Promise.all([
          getPersonnel(),
          getRoles(true), // Sadece aktif roller
          getUsers().catch(() => []),
        ]);
  ...
  const openEdit = (person) => {
    ...
    setForm({
      ...
      password: '',
      systemRole: existingUser?.role || 'user',
  ...
  const saveForm = async (e) => {
    ...
    try {
      ...
      if (editing) {
        ...
        // Mevcut kullanıcı hesabı varsa ve admin düzenliyorsa, rol/izin güncelle
        // Kullanıcı hesabı varsa rolü güncelle (izinler rol bazlı)
        if (form.hasExistingUser && form.existingUserId && isAdmin) {
          try {
            const updatedUser = await updateUser(form.existingUserId, {
              displayName: `${form.ad} ${form.soyad}`.trim(),
              role: form.systemRole,
            });
            setUsers((prev) => prev.map((u) => (u.id === updatedUser.id ? updatedUser : u)));
          } catch (userErr) {
            setError(`Personel kaydedildi ancak kullanıcı güncellenemedi: ${userErr.message}`);
          }
        }
  ...
  // Şifre değiştir (düzenleme modalı içinde)
  ...
  // Personele bağlı kullanıcının sistem rolünü getir
  const getUserRole = (person) => {
    const linkedUser = users.find(u => u.personnelId === person.id);
    if (!linkedUser) return null;
    // Rol adını roles.json'dan bul
    const role = roles.find(r => r.ad === linkedUser.role && !r.deleted);
    return role ? role.ad : linkedUser.role;
  };

  // Aktif ve silinmemiş roller (dropdown için)
  const activeRoles = roles.filter(r => r.aktifMi && !r.deleted);

  // Columns - useMemo kaldırıldı (closure sorunlarını önlemek için)
  const columns = [
    {
      accessor: 'ad',
      label: 'Ad Soyad',
    ...
    { accessor: 'telefon', label: 'Telefon' },
    {
      ...
      label: 'Sistem Rolü',
      render: (_, row) => {
        const roleName = getUserRole(row);
        if (!roleName) {
          return <span className="text-muted" style={{ fontSize: 12 }}>Hesap yok</span>;
        }
        return <span className="badge badge-info">{roleName}</span>;
      },
    },
    {
      accessor: 'aktifMi',
    ...
    <div>
      ...
      >
        <form onSubmit={saveForm}>
          ...
          {/* Kullanıcı Hesabı Bölümü - Sadece Admin */}
          {isAdmin && (
            <div style={{ 
              marginTop: 16, 
              paddingTop: 16, 
              ...
              <div style={{ 
                ...
                <StatusIcon icon="admin_panel_settings" style={{ fontSize: 20 }} />
                ...
                <div style={{ 
                  ...
                  <div className="form-group" style={{ marginBottom: 12 }}>
                    <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                      <StatusIcon icon="security" style={{ fontSize: 16 }} /> Sistem Rolü
                    </label>
                    <select
                    ...
                    >
                      {activeRoles.map(role => (
                        <option key={role.id} value={role.ad}>
                          {role.ad}
                        </option>
                      ))}
                    </select>
                    <div style={{ fontSize: 11, color: 'var(--color-text-light)', marginTop: 4 }}>
                      {activeRoles.find(r => r.ad === form.systemRole)?.aciklama || 'Roller sayfasından yönetilir'}
                    </div>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    {!showPasswordField ? (
                      ...
                      >
                        <StatusIcon icon="lock" style={{ fontSize: 16 }} />
                        Şifre Değiştir
                      </button>
                    ) : (
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (form.existingUserId) {
                              handleChangePassword(form.existingUserId);
                            }
                          }}
                        >
                ...
                <>
                  <label className="form-label" style={{ 
                    ...
                    <span style={{ fontSize: 15, display: 'flex', alignItems: 'center', gap: 6 }}>
                      <StatusIcon icon="vpn_key" style={{ fontSize: 18 }} /> Sisteme giriş için kullanıcı hesabı oluştur
                    </span>
                  </label>
                  ...
                  {form.createUser && (
                    <div style={{ 
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                        <div className="form-group" style={{ marginBottom: 0 }}>
                          <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                            <StatusIcon icon="person" style={{ fontSize: 16 }} /> Kullanıcı Adı <span style={{ color: 'var(--color-danger)' }}>*</span>
                          </label>
                          <input
                            type="text"
                        ...
                        <div className="form-group" style={{ marginBottom: 0 }}>
                          <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                            <StatusIcon icon="lock" style={{ fontSize: 16 }} /> Şifre <span style={{ color: 'var(--color-danger)' }}>*</span>
                          </label>
                          <input
                            type="password"
                      ...
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                          <StatusIcon icon="security" style={{ fontSize: 16 }} /> Sistem Rolü <span style={{ color: 'var(--color-danger)' }}>*</span>
                        </label>
                        <select
                          className="form-select"
                          value={form.systemRole}
