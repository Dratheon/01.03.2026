...
def _setup_test_data():
    ...
    # Ensure test personnel exists for assignment tests
    personnel_path = TEST_DATA_DIR / "personnel.json"
    personnel = json.loads(personnel_path.read_text(encoding="utf-8")) if personnel_path.exists() else []
    if not any(p.get("id") == "PER-001" for p in personnel):
        personnel.append({
            "id": "PER-001",
            "ad": "Test",
            "soyad": "Personel",
        ...
        personnel_path.write_text(json.dumps(personnel, ensure_ascii=False, indent=2), encoding="utf-8")
